...
function uploadSuccess(file, serverData) {
    try {
        ...
        if (jsonstr.status == '1') {
            //如果是单文件上传，则赋值相应的表单
            if (this.customSettings.button_action == SWFUpload.BUTTON_ACTION.SELECT_FILE) {
                $(".focus-photo").val(jsonstr.path);
                var newLi = $('<img src="' + jsonstr.path + '"  width="153" height="150"/>');
                $(".body_hy_xinxi_left").html(newLi);
...
function addImage(targetObj, originalSrc, thumbSrc) {
    alert(thumbSrc);
    //插入到相册UL里面
    var newLi = $('<img src="' + thumbSrc + '" bigsrc="' + originalSrc + '"  width="153" height="150"/>');
    newLi.appendTo(".body_hy_xinxi_left");
    $("#ctl00$ContentPlaceHolder1$txtAvatar").val(thumbSrc);
    alert(newLi);
}
//设置相册封面
    ...
    var m = $.dialog({
        ...
        button: [{
            ...
            callback: function () {
                ...
                parentObj.parent().parent().find("li input[name='hid_photo_remark']").val(remarkObj);
                parentObj.parent().parent().find("li .img-box .remark i").html(remarkObj);
            }
        }, {
            name: '单张描述',
            callback: function () {
                ...
                hidRemarkObj.val(remarkObj);
                parentObj.siblings(".img-box").children(".remark").children("i").html(remarkObj);
            },
            focus: true
        }]
    });
