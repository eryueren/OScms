...
namespace OS.Web.admin.article
{
    public partial class article_edit : Web.UI.ManagePage
    {
        ...
        {
            CreateOtherField(this.category_id); //动态生成相应的扩展字段
        }
        ...
        {
            List<Model.contents.article_attribute_field> ls = new BLL.contents.article_attribute_field().GetModelList(this.category_id, "is_sys=0");
            if (ls.Count > 0)
            {
        ...
        {
            DataTable dt = new BLL.contents.article_attribute_field().GetList(_channel_id, "").Tables[0];
            ...
            if (albumArr != null && albumArr.Length > 0)
            {
                ...
                for (int i = 0; i < albumArr.Length; i++)
                {
                    ...
                    if (imgArr.Length == 3)
                    {
                        ...
                        {
                            ls.Add(new Model.contents.article_albums { original_path = imgArr[1], thumb_path = imgArr[2], remark = remarkArr[i] });
                        }
                        else
                        {
                            ls.Add(new Model.contents.article_albums { original_path = imgArr[1], thumb_path = imgArr[2] });
                        }
                    }
                }
                model.albums = ls;
                ...
                && attachFileNameArr.Length > 0 && attachFilePathArr.Length > 0 && attachFileSizeArr.Length > 0 )
            {
                ...
                for (int i = 0; i < attachFileNameArr.Length; i++)
                {
                   ...
                   // int _point = Utils.StrToInt(attachPointArr[i], 0);
                    ls.Add(new Model.contents.article_attach { file_name = attachFileNameArr[i], file_path = attachFilePathArr[i], file_size = fileSize, file_ext = fileExt });
                }
                model.attach = ls;
            }
            #endregion
            ...
            if (albumArr != null)
            {
                ...
                for (int i = 0; i < albumArr.Length; i++)
                {
                    ...
                    if (imgArr.Length == 3)
                    {
                        if (!string.IsNullOrEmpty(remarkArr[i]))
                        {
                            ls.Add(new Model.contents.article_albums { id = img_id, article_id = _id, original_path = imgArr[1], thumb_path = imgArr[2], remark = remarkArr[i] });
                        }
                        else
                        {
                            ls.Add(new Model.contents.article_albums { id = img_id, article_id = _id, original_path = imgArr[1], thumb_path = imgArr[2] });
                        }
                    }
                }
                model.albums = ls;
         ...
         //   string[] attachPointArr = Request.Form.GetValues("txt_attach_point");
                ...
                && attachIdArr.Length > 0 && attachFileNameArr.Length > 0 && attachFilePathArr.Length > 0 && attachFileSizeArr.Length > 0 )
            {
                ...
                for (int i = 0; i < attachFileNameArr.Length; i++)
                {
                    ...
                    string fileExt = Utils.GetFileExt(attachFilePathArr[i]);
                   // int _point = Utils.StrToInt(attachPointArr[i], 0);
                    ls.Add(new Model.contents.article_attach { id = attachId, article_id = _id, file_name = attachFileNameArr[i], file_path = attachFilePathArr[i], file_size = fileSize, file_ext = fileExt, });
                }
                model.attach = ls;
            }
            #endregion
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == OSEnums.ActionEnum.Edit.ToString()) //修改
            {
                ...
                PageSuccessMsg("修改信息成功", "", "article_list.aspx?category_id=" + this.category_id);
