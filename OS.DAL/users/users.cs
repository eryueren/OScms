...
namespace  OS.DAL.users {
	...
	public partial class users {
		...
		/// 根据用户名密码返回一个实体
		/// </summary>
		public Model.users.users GetModel(string user_name, string password, int emaillogin, int mobilelogin) {
			StringBuilder strSql = new StringBuilder();
		...
		/// 根据用户名返回一个实体
		/// </summary>
		public Model.users.users GetModel(string user_name) {
			StringBuilder strSql = new StringBuilder();
		...
		/// 根据用户名返回一个实体
		/// </summary>
		public Model.users.users GetModelByEmail(string email) {
			StringBuilder strSql = new StringBuilder();
			strSql.Append("select id from " + databaseprefix + "users");
			strSql.Append(" where email=@email and status=0");
			SqlParameter[] parameters = {
					new SqlParameter("@email", SqlDbType.NVarChar,100)};
			parameters[0].Value = email;
		...
		public DataSet GetList(int Top, string strWhere, string filedOrder) {
			...
			strSql.Append(" id,group_id,user_name,password,salt,email,universities,professional,membershipdues,goodat,workat,industry ,nick_name,avatar,sex,birthday,telphone,mobile,qq,address,safe_question,safe_answer,amount,point,exp,status,reg_time,reg_ip ");
