...
namespace  OS.DAL.contents
{
    public class article_images_size
    {
        ...
        /// </summary>
         public void DeleteList(SqlConnection conn, SqlTransaction trans, List<Model.contents.article_images_size> models, int category_id)
         {
             StringBuilder idList = new StringBuilder();
             if (models != null)
             {
                 foreach (Model.contents.article_images_size modelt in models)
                 {
                     if (modelt.id > 0)
                     {
         ...
         {
             List<Model.contents.article_images_size> modelList = new List<Model.contents.article_images_size>();
         ...
         /// 得到一个对象实体
         /// </summary>
         public Model.contents.article_images_size DataRowToModel(DataRow row)
         {
             Model.contents.article_images_size model = new Model.contents.article_images_size();
             if (row != null)
             {
