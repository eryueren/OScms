...
namespace  OS.DAL.contents
{
	...
	public partial class article_albums
	{
        ...
        /// 获得数据列表
        /// </summary>
        public List<Model.contents.article_albums> GetList(int article_id)
        {
            List<Model.contents.article_albums> modelList = new List<Model.contents.article_albums>();
            ...
            if (rowsCount > 0)
            {
                Model.contents.article_albums model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new Model.contents.article_albums();
        ...
        /// </summary>
        public void DeleteList(SqlConnection conn, SqlTransaction trans, List<Model.contents.article_albums> models, int article_id)
        {
            StringBuilder idList = new StringBuilder();
            if (models != null)
            {
                foreach (Model.contents.article_albums modelt in models)
                {
                    if (modelt.id > 0)
                    {
        ...
        /// 删除相册图片
        /// </summary>
        public void DeleteFile(List<Model.contents.article_albums> models)
        {
            if (models != null)
            {
                foreach (Model.contents.article_albums modelt in models)
