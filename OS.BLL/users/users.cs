...
namespace OS.BLL.users
{
    ...
    public partial class users
    {
        ...
        private readonly DAL.users.users dal;
        public users()
        {
            dal = new DAL.users.users(siteConfig.sysdatabaseprefix);
        }
        ...
        /// <returns></returns>
        public Model.users.users GetModel(string user_name, string password, int emaillogin, int mobilelogin, bool is_encrypt)
        {
            //检查一下是否需要加密
            if (is_encrypt)
            {
